...
void Game::Init()
{
    ...
    //view = glm::translate(view, glm::vec3(PlayerBall->Position.x, PlayerBall->Position.y, 0.0f));
    //view = glm::scale(view, glm::vec3(2.0f, 2.0f, 1.0f));
    //view = glm::translate(view, glm::vec3(-PlayerBall->Position.x, -PlayerBall->Position.y, 0.0f));
    //view = glm::lookAt(glm::vec3(PlayerBall->Position.x, PlayerBall->Position.y, 0.0f), glm::vec3(PlayerBall->Position.x, PlayerBall->Position.y, 0.0f), glm::vec3(0.0f, 1.0f, 0.0f));
    ...
    //glm::vec2 playerPos = glm::vec2(this->Width / 2.0f - BALL_RADIUS / 2.0f, this->Height / 2.0f - BALL_RADIUS / 2.0f);
    glm::vec2 playerPos = glm::vec2(400, 450);
    PlayerBall = new BallObject(playerPos, BALL_RADIUS, INITIAL_BALL_VELOCITY, ResourceManager::GetTexture("face"));
    //view = glm::scale(view, glm::vec3(2.0f, 2.0f, 1.0f));
    //glViewport(PlayerBall->Position.x, PlayerBall->Position.y, this->Width, this->Height);
    //glm::mat4 view = glm::mat4(1.0f);
    //view = glm::lookAt(glm::vec3(PlayerBall->getXpos(), PlayerBall->getYpos(), 1.0f), glm::vec3(PlayerBall->getXpos(), PlayerBall->getYpos(), 0.0f), glm::vec3(0.0f, 1.0f, 0.0f));
    //ResourceManager::GetShader("sprite").SetMatrix4("view", view);
    //ResourceManager::GetShader("particle").SetMatrix4("view", view);
    /*
    glm::vec2 playerPos = glm::vec2(this->Width / 2.0f - PLAYER_SIZE.x / 2.0f, this->Height / 2.0f - PLAYER_SIZE.y);
    Player = new GameObject(playerPos, PLAYER_SIZE, ResourceManager::GetTexture("paddle"));
    glm::vec2 ballPos = playerPos + glm::vec2(PLAYER_SIZE.x / 2.0f - BALL_RADIUS, -BALL_RADIUS * 2.0f);
    Ball = new BallObject(ballPos, BALL_RADIUS, INITIAL_BALL_VELOCITY, ResourceManager::GetTexture("face"));
    */
    // audio
    SoundEngine->play2D("audio/Country_Roads.mp3", true);
}
...
void Game::Update(float dt)
{
    ...
    this->DoCollisions();
    // update particles
    Particles->Update(dt, *PlayerBall, 2, glm::vec2(PlayerBall->Radius / 2.0f));
    // update PowerUps
    ...
    if (PlayerBall->Position.y >= this->Height) // did ball reach bottom edge?
    {
        --this->Lives;
...
void Game::ProcessInput(float dt)
{
    ...
    if (this->State == GAME_ACTIVE)
    {
        ...
        {
            if (PlayerBall->Position.x <= this->Width - PlayerBall->Size.x)
            {
                PlayerBall->Position.x += velocity;
                //if (Ball->Stuck)
                //    Ball->Position.x += velocity;
        ...
        {
            if (PlayerBall->Position.y <= this->Height - PlayerBall->Size.y)
            {
                PlayerBall->Position.y += velocity;
                //if (Ball->Stuck)
                //    Ball->Position.x += velocity;
            }
        }
...
void Game::Render()
{
    //glm::mat4 projection = glm::translate(projection, glm::vec3(PlayerBall->getXpos(), PlayerBall->getYpos(), 0.0f));
    //ResourceManager::GetShader("sprite").SetMatrix4("projection", projection);
    //ResourceManager::GetShader("particle").SetMatrix4("projection", projection);
    if (this->State == GAME_ACTIVE || this->State == GAME_MENU || this->State == GAME_WIN)
    {
        //glm::mat4 projection = glm::translate(projection, glm::vec3(PlayerBall->Position.x, PlayerBall->Position.y, 0.0f));
        //ResourceManager::GetShader("sprite").SetMatrix4("projection", projection);
        //ResourceManager::GetShader("particle").SetMatrix4("projection", projection);
        //glm::mat4 projection = glm::lookAt(glm::vec3(200.0f, 150.0f, 0.0f), glm::vec3(PlayerBall->Position.x, PlayerBall->Position.y, 0.0f), glm::vec3(0.0f, 1.0f, 0.0f));
        //glViewport(PlayerBall->Position.y, PlayerBall->Position.x, this->Height, this->Width);
        //ResourceManager::GetShader("sprite").SetMatrix4("projection", projection);
        //ResourceManager::GetShader("particle").SetMatrix4("projection", projection);
    ...
    //projection = glm::translate(projection, glm::vec3(-PlayerBall->Position.x, -PlayerBall->Position.y, 0.0f));
    //ResourceManager::GetShader("sprite").SetMatrix4("projection", projection);
    //ResourceManager::GetShader("particle").SetMatrix4("projection", projection);
}
...
void Game::ResetPlayer()
{
    ...
    PlayerBall->Radius = BALL_RADIUS;
    PlayerBall->Position = glm::vec2(this->Width / 2.0f - BALL_RADIUS / 2.0f, this->Height / 2.0f - BALL_RADIUS);
...
void Game::DoCollisions()
{
    for (GameObject& box : this->Levels[this->Level].Bricks)
    {
        if (!box.Destroyed)
        {
            Collision collision = CheckCollision(*PlayerBall, box);
            if (std::get<0>(collision)) // if collision is true
            {
                ...
                if (!(PlayerBall->PassThrough && !box.IsSolid)) // don't do collision resolution on non-solid bricks if pass-through is activated
                {
                    if (dir == LEFT || dir == RIGHT) // horizontal collision
                    {
                        ...
                        if (dir == LEFT)
                            PlayerBall->Position.x += penetration; // move ball to right
                        else
                            PlayerBall->Position.x -= penetration; // move ball to left;
                    }
                    else // vertical collision
                    {
                        ...
                        if (dir == UP)
                            PlayerBall->Position.y -= penetration; // move ball bback up
                        else
